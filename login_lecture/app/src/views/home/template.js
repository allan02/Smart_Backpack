module.exports = {
  ...
  DATA: function (title, content, date, hit, name, list) {
    //수정함 원래는 date없음 + list추가
    return `
    <!doctype html>
    <html>
    <script src="https://ajax.googleapis.com/ajax/libs/angularjs/1.6.9/angular.min.js">
        ...
        <div id="wrapper">
          ...
          <section id="three" class="wrapper style1 fullscreen fade-up">
            <div class="inner">
              ...
              <div>
                  <center>
                    <a href ="/write" class = "button">수정</a>
                    <a href = "/board" class = "button" style = "margin-left:15px" onclick="alert('정말 삭제하시겠습니까?'); return "/board";">삭제</a>
                  </center>
              </div>
                <div>
  ...
  list: function (filelist, datelist, hitlist) {
    ...
    while (i < filelist.length) {
      ...
      list =
        list +
        `<tr><td><center>${No}</center></td><td><a href="/data/${filelist[i]}">${filelist[i]}</a></td><td><center>${datelist[i]}</center></td><td><center>${hitlist[i]}</center></td></tr>`;
      i = i + 1;
    }
    list = list + "</tbody>";
    return list;
  },
  commentList: function (nameArr, dateArr, contentsArr, next) {
    //댓글------------------------------------------------------------
    var cL = "<table>";
    var i = 0;
    var fs = require("fs");
    ...
    while (i < contentsArr.length) {
      ...
      cL =
        cL +
        `<p class="comment_writer"><b>${nameArr[i]}</b></p><p class="comment_content">${contentsArr[i]}</p><p class="comment_date">${dateArr[i]}</p>`;
      cL =
        cL +
        `<p style="float:right"><a>수정</a><a style = "margin-left:15px" onclick="alert('정말 삭제하시겠습니까?'); return "/board";">삭제</a></p>`;
      cL = cL + "</td></tr>";
      i = i + 1;
    }
    cL = cL + "</table>";
